    ...
    public class BL_HBTL
    {
        ...
        public DataTable HienThiHOADON(string Ngay1, string Ngay2, string KhachHang)
        {
            string Query = "SELECT a.MAHD,a.MAKH,b.TENKH,a.NGAYHD,a.MANV,c.TENNV,TONGTIEN,TRANGTHAI,DIENGIAI FROM HOADON a INNER JOIN KHACHHANG b ON a.MAKH = b.MAKH INNER JOIN NHANVIEN c ON a.MANV = c.MANV " +
                "WHERE a.Isactive=1 AND a.MACT = 'TL' AND " +
                "(a.MAKH = '"+KhachHang+ "' OR ISNULL('"+KhachHang+"','')='') AND " +
                "NGAYHD BETWEEN '" + Ngay1 + "' AND '" + Ngay2 + "'";
            return DBConnect.Load(Query);
        }

        public DataTable HienThiHBTLDinhChi()
        {
            string Query = "SELECT a.MAHD,a.MAKH,b.TENKH,a.NGAYHD,a.MANV,c.TENNV,TONGTIEN,TRANGTHAI,DIENGIAI FROM HOADON a INNER JOIN KHACHHANG b ON a.MAKH = b.MAKH INNER JOIN NHANVIEN c ON a.MANV = c.MANV " +
                           "WHERE a.MACT = 'TL' AND a.Isactive=0 ";
            return DBConnect.Load(Query);
        }
        ...
        {
            string Query = "SELECT * FROM KHACHHANG  WHERE Isactive=1 AND TENKH LIKE N'%" + TenKH + "%'";
            return DBConnect.Load(Query);
        }
        ...
        public DataTable HienThiVatTuDaMua(string TenVT, string MaKH)
                {
                    string Query = "SELECT distinct ct.MAVT, vt.TENVT, vt.MANHOM, vt.DVT, vt.MOTA, vt.KICHTHUOC, vt.KHOILUONG, vt.DONGIA " +
                                            "FROM CHITIETHOADON ct INNER JOIN HOADON ct0 on ct.MAHD=ct0.MAHD " +
                                                                  "INNER JOIN VATTU vt on vt.MAVT=ct.MAVT  " +
                                            "WHERE ct0.Isactive=1 AND ct0.MAKH= N'"+ MaKH+"'"+
                                                                " AND vt.TenVT LIKE N'%" + TenVT + "%'";
                    return DBConnect.Load(Query);
                }

        //public void ThemHoaDon(string MaHD, string makh, string NgayHD, string manv, decimal TongTien, string DienGiai)
        //{
        //    string str = "INSERT INTO HoaDon(MaHD,makh,NgayHD,manv,TongTien,TrangThai,DIENGIAI,MACT)  VALUES ( '" + MaHD + "','" + makh + "','" + NgayHD + "','" + manv + "'," + TongTien + ",N'Chưa thanh toán"+", N'" + DienGiai + "','TL')";
